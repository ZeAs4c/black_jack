...
class _BlackJackScreenState extends State<BlackJackScreen> {
  ...
  //Добавляем все карты, когда приложение билдится первый раз
  @override
  void initState() {
    super.initState();
  ...
  void changeCards() {
    ...
    String cardTwoKey = playingCards.keys.elementAt(random.nextInt(
        playingCards.length));
    playingCards.removeWhere((key, value) => key == cardTwoKey);

    String cardThreeKey = playingCards.keys.elementAt(random.nextInt(
        playingCards.length));
    playingCards.removeWhere((key, value) => key == cardThreeKey);

    String cardFourKey = playingCards.keys.elementAt(random.nextInt(
        playingCards.length));
    ...
    dealersCards.add(Image.asset(
        dealersFirstCards!)); // Знак вопроса говорит о том что мы уверены что в карта есть и не является mull
  ...
  void addCard() {
    ...
    if(playingCards.isNotEmpty){
      ...
      playingCards.removeWhere((key, value) => key == cardKey); // Убираем карту с колоды
      setState(() {
        myCards.add(
          Image.asset(cardKey),
        );
